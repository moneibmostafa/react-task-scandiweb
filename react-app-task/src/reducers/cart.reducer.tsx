    ...
    switch (action.type) {
        ...
        case navbarConstants.ADD_TO_CART:
            productFound = state.cartItems.findIndex((obj) => {
              return obj.item.name === action.item.name && obj.selectedAttribute === action.selectedAttribute
            })
            if(productFound > -1) state.cartItems[productFound].count++;
            else state.cartItems = [...state.cartItems, {item: action.item, count: 1, selectedAttribute: action.selectedAttribute}];
        ...
        case cartConstants.CHANGE_ITEM_COUNT:
            productFound = state.cartItems.findIndex((obj) => {
                return obj.item.name === action.itemName && obj.selectedAttribute === action.selectedAttribute
            })
            if(productFound > -1) {
